...
#include "robocluedo_rosplan_actions/move_to.h"
...
// class constructor
RP_rcl_move_to::RP_rcl_move_to( ros::NodeHandle& nh_ ) : 
	RPActionInterface( ),
	nh( nh_ )
{
	// action feedback
...
bool RP_rcl_move_to::concreteCallback( const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg )
{
	std::string from = msg->parameters[0].value;
	std::string to = msg->parameters[1].value;
	ROS_INFO_STREAM("["<<action_name<<"] "<<"(move-to ?" << from << " ?" << to << ")");
	
	robocluedo_rosplan_msgs::NavigationCommand nav_srv;
	nav_srv.request.waypoint = to;
